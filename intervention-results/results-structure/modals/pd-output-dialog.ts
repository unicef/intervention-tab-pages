    ...
    return html`
      ...
      >
        <div class="unassociated-warning" ?hidden="${!this.unassociated || this.hideCpOutputs}">
          <div>
            <iron-icon icon="warning"></iron-icon>${translate('PD_OUTPUT_DIALOG.ASSOCIATE_PROMPT')}
          </div>
          ${!this.cpOutputs.length
            ? html`
                <div>
                  <br /><iron-icon icon="warning"></iron-icon> ${translate(
                    'PD_OUTPUT_DIALOG.ASSOCIATE_MSG'
                  )}
                </div>
              `
            : ''}
        ...
        <div class="container layout vertical">
          <paper-input
            ...
            label=${translate('PD_OUTPUT_DIALOG.PD_OUTPUT_NAME')}
            placeholder="&#8212;"
  ...
  processRequest(): void {
    ...
    sendRequest({
      ...
      .catch((error) => {
        ...
        fireEvent(this, 'toast', {text: getTranslation('PD_OUTPUT_DIALOG.ERR_SAVE_PD_OUTPUT')});
      });
  }
}
