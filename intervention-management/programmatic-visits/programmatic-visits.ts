...
export class ProgrammaticVisits extends CommentsMixin(ComponentBaseMixin(RepeatableDataSetsMixin(LitElement))) {
  ...
  _yearChanged(event: CustomEvent, index: number) {
    ...
    if (this.isAlreadySelected(yearSelected, index, 'year')) {
      fireEvent(this, 'toast', {
        text: (translate('INTERVENTION_MANAGEMENT.PROGRAMATIC_VISITS.YEAR_SELECTED_ERR') as unknown) as string,
        showCloseBtn: true
      });
  ...
  _addNewPlannedVisit() {
    if (!this.validate()) {
      fireEvent(this, 'toast', {
        text: (translate(
          'INTERVENTION_MANAGEMENT.PROGRAMATIC_VISITS.ALREADY_ADDED_PLANNED_VISIT'
