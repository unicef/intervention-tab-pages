...
export class UpdateFrNumbers extends RepeatableDataSetsMixin(LitElement) {
  ...
  render() {
    ...
    return html`
      ...
      <etools-dialog
        id="frsDialog"
        size="md"
        dialog-title=${translate('INTERVENTION_METADATA.FUND_RESERVATIONS.FR_DIALOG.ADD_UPDATE_FR_NUMBERS')}
      ...
      >
            ...
            <div class="row-h item-container">
              ...
              <div class="item-content">
                <div class="row-h">
                  ...
                  <paper-input
                    .id="fr-nr-${index}"
                    label=${translate('INTERVENTION_METADATA.FUND_RESERVATIONS.FR_DIALOG.FR_NUMBER')}
                    .value="${item.fr_number}"
                    placeholder="&#8212;"
                    allowed-pattern="[0-9]"
                    required
                    error-message=${translate('INTERVENTION_METADATA.FUND_RESERVATIONS.FR_DIALOG.FILL_FR_NUMBER')}
        ...
        <div class="${(this.data || []).length ? 'hidden' : 'row-h'}">
          ${translate('INTERVENTION_METADATA.FUND_RESERVATIONS.FR_DIALOG.NO_FUND_RESERVATIONS_ADDED')}
        </div>

        <div class="row-h">
          <paper-button class="secondary-btn" @click="${() => this._addNewFundReservation()}">
            <iron-icon icon="add"></iron-icon>
            ${translate('INTERVENTION_METADATA.FUND_RESERVATIONS.FR_DIALOG.ADD_FR_NUM')}
          </paper-button>
        </div>
      </etools-dialog>
    `;
  ...
  deleteConfirmationMessage = (translate(
    'INTERVENTION_METADATA.FUND_RESERVATIONS.FR_DIALOG.DELETE_FR_PROMPT'
